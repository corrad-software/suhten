...
<template>
  <AdminLayout>
    <div class="mx-auto max-w-7xl space-y-4">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-0.5 text-sm text-slate-500">
            AI coding reference in <code class="rounded-sm bg-slate-100 px-1 py-0.5 text-xs">docs/ai-guidelines/</code> &mdash; edit
            <code class="rounded-sm bg-slate-100 px-1 py-0.5 text-xs">CLAUDE.md</code> and auto-sync to
            <code class="rounded-sm bg-slate-100 px-1 py-0.5 text-xs">.cursorrules</code>,
            <code class="rounded-sm bg-slate-100 px-1 py-0.5 text-xs">AGENTS.md</code>
          </p>
        </div>
        <!-- Mode toggle -->
      ...
      <template v-else>
        <!-- ───── Sync Status ───── -->
        <article v-if="syncFiles.length > 0" class="rounded-lg border border-slate-200 bg-white shadow-xs">
        ...
        <template v-if="mode === 'view'">
          ...
          <div class="space-y-1">
            <article
              ...
              :key="index"
              class="rounded-lg border border-slate-200 bg-white shadow-xs overflow-hidden"
            >
              <!-- Accordion header -->
              <button
