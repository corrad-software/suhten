...
const childRowClass = computed(() =>
  isCompact.value
    ? "block rounded-md px-3 py-0.5 text-[13px] leading-tight transition-all hover:bg-(--accent-50)"
...
function itemClass(path: string) {
  if (isActive(path)) {
    return "border border-(--accent-200) bg-(--accent-50) font-medium text-(--accent-700)";
  }
  ...
  <div class="min-h-screen bg-[#f8f9fb]">
    <header class="sticky top-0 z-40 flex h-10 items-center justify-between border-b border-slate-200 bg-white px-5">
      <div class="flex items-center gap-1">
        ...
        <div
          v-else
          class="flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-md bg-linear-to-br from-(--accent-600) to-(--accent-500)"
        >
      ...
      <div class="flex items-center self-stretch">
        ...
        <router-link
          ...
          class="group relative flex h-full items-center gap-2 px-4 transition-colors hover:bg-(--accent-600)"
        >
          <div
            class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-(--accent-600) to-(--accent-500) text-[10px] font-semibold text-white"
          >
            {{ userInitials }}
          </div>
        ...
        <div ref="chatDropdownRef" class="relative flex h-full items-stretch">
          <button
            class="group relative flex h-full items-center px-4 text-slate-500 transition-colors hover:bg-(--accent-600) hover:text-white"
              ...
              >
                ...
                >
                  ...
                  <p
                    ...
                    :class="msg.from === 'user' ? 'text-white/70' : 'text-slate-400'"
                  >{{ msg.time }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="border-t border-slate-200 px-3 py-2.5">
              <div class="flex items-center gap-2">
                <input
                  ...
                  class="flex-1 rounded-md border border-slate-200 px-3 py-1.5 text-xs text-slate-700 outline-hidden transition-colors focus:border-(--accent-400) focus:ring-1 focus:ring-(--accent-400)"
                  @keydown.enter="sendChatMessage"
                />
                <button
                  class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-(--accent-600) text-white transition-colors hover:bg-(--accent-700)"
    ...
    <div class="flex flex-col md:flex-row">
      ...
      >
        <button
          class="absolute -right-3.5 top-10 z-40 hidden h-7 w-7 items-center justify-center rounded-full border border-slate-200 bg-(--accent-600) text-white shadow-md transition-all hover:bg-(--accent-700) hover:shadow-lg md:flex"
          ...
          <div v-for="(group, gi) in menuStore.resolvedMenu" :key="group.id">
            ...
            <div v-for="item in group.items" :key="item.id" class="mb-0.5">
              <button
                ...
                type="button"
                class="group relative flex w-full items-center rounded-lg text-left font-medium transition-all hover:bg-(--accent-50)"
                :class="[
                  isCollapsed ? collapsedRowBaseClass : rowBaseClass,
                  isCollapsed && isNodeActive(item) ? 'md:border md:border-(--accent-200) md:bg-(--accent-50) md:text-(--accent-700) md:font-medium text-slate-900' : 'text-slate-900',
              ...
              >
                <component
                  ...
                  :class="[
                    ...
                    isCollapsed && isNodeActive(item) ? 'md:text-(--accent-700) text-slate-700' : isNodeActive(item) ? 'text-slate-900' : 'text-slate-400 group-hover:text-(--accent-600)'
                  ]"
                />
              ...
              <router-link
                v-else
                :to="item.to"
                class="group relative flex items-center rounded-lg font-medium transition-all hover:bg-(--accent-50)"
                :class="[
                  isCollapsed ? collapsedRowBaseClass : rowBaseClass,
                  isCollapsed && isActive(item.to) ? 'md:border md:border-(--accent-200) md:bg-(--accent-50) md:text-(--accent-700) md:font-medium text-slate-900' : 'text-slate-900',
              ...
              >
                <component
                  ...
                  :class="[
                    isCollapsed ? 'md:h-5 md:w-5 h-4 w-4' : 'h-4 w-4',
                    isCollapsed && isActive(item.to) ? 'md:text-(--accent-700) text-slate-700' : isActive(item.to) ? 'text-slate-900' : 'text-slate-400 group-hover:text-(--accent-600)'
                  ]"
                />
                <span class="flex-1" :class="isCollapsed ? 'md:hidden' : ''">{{ item.label }}</span>
                <span
              ...
              >
                <template v-for="child in item.children" :key="child.id">
                  <button
                    ...
                    type="button"
                    class="flex w-full items-center rounded-md text-left transition-all hover:bg-(--accent-50)"
