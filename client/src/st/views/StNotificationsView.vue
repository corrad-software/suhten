  ...
  <div class="space-y-4">
    ...
    >
      <template v-if="notifications.unreadCount > 0" #action>
        <button
          class="flex shrink-0 items-center gap-1.5 rounded-md border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50"
    ...
    <div>
      <p v-if="notifications.forCurrentPersona.length === 0" class="py-12 text-center text-sm text-slate-400">
        {{ ts("st.notif.empty") }}
      </p>
      <button
        v-for="n in notifications.forCurrentPersona"
        :key="n.id"
        class="flex w-full items-start gap-3 border-b border-slate-100 py-3 text-left transition-colors last:border-0 hover:bg-slate-50/60"
