  ...
  <div class="space-y-5">
    <div class="flex items-start gap-3">
      <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--accent-50)]">
        <Search class="h-5 w-5 text-[var(--accent-700)]" />
      </div>
      <div>
        <h1 class="text-xl font-semibold text-slate-900">{{ ts("st.search.title") }}</h1>
        <p class="text-sm text-slate-500">{{ ts("st.search.subtitle") }}</p>
      </div>
    </div>
    ...
    >
      ...
      <div class="flex flex-col gap-3 sm:flex-row">
        <div class="relative flex-1">
          ...
          <input
            ...
            class="w-full rounded-lg border border-slate-300 py-2.5 pl-10 pr-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-[var(--accent-500)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-ring)]"
          />
        </div>
        <button
          type="submit"
      ...
      <p class="mt-2 text-xs text-slate-400">{{ ts("st.search.hint") }}</p>

      <div class="mt-4 flex flex-wrap items-center gap-2">
        <Sparkles class="h-3.5 w-3.5 text-slate-400" />
        <span class="text-xs text-slate-500">{{ ts("st.search.quickTitle") }}:</span>
        <button
          v-for="item in quick"
          :key="item.q"
          type="button"
    ...
    <template v-if="submitted">
      <div class="flex items-baseline justify-between gap-2">
        <h2 class="text-sm font-semibold text-slate-800">
          ...
          <span class="font-normal text-slate-400">({{ hits.length }})</span>
        </h2>
      </div>

      <p v-if="hits.length === 0" class="rounded-xl border border-dashed border-slate-200 bg-white px-4 py-12 text-center text-sm text-slate-400">
        {{ ts("st.search.noHit") }}
      </p>
          ...
          <ul v-else class="divide-y divide-slate-100">
            <li v-for="hit in apps" :key="hit.id" class="py-3 hover:bg-slate-50/60">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] text-slate-600">{{ hit.moduleCode }}</span>
                <RegStatusBadge :status="hit.status" />
              </div>
              <p class="mt-1 font-mono text-xs text-slate-600">{{ hit.refNo }}</p>
              <p class="text-sm font-medium text-slate-900">{{ hit.applicantName }}</p>
        ...
        <section class="lg:pl-6">
          ...
          <ul v-else class="divide-y divide-slate-100">
            <li v-for="hit in regs" :key="hit.id" class="py-3 hover:bg-slate-50/60">
              ...
              <p class="mt-1 font-mono text-xs text-slate-600">{{ hit.certificateNo }}</p>
              <p class="text-sm font-medium text-slate-900">{{ hit.holderName }}</p>
              <p class="text-xs text-slate-500">
