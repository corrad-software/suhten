  ...
  <ol class="space-y-0">
    ...
    >
      <div class="flex flex-col items-center">
        ...
        <span v-if="i < entries.length - 1" class="mt-0.5 w-px flex-1 bg-slate-200" />
      </div>
      <div class="min-w-0 pb-1">
        <p class="text-sm font-medium text-slate-800">{{ e.action }}</p>
        <p class="text-xs text-slate-500">{{ e.actorName }} · {{ roleLabel(e.actorRole) }} · {{ fmt(e.at) }}</p>
        <p v-if="e.note" class="mt-1 rounded-md bg-amber-50 px-2 py-1 text-xs text-amber-700">{{ e.note }}</p>
      </div>
    </li>
  </ol>
</template>
