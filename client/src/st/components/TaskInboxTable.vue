        ...
        >
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              ...
              <span
                v-if="workflow.byId(item.applicationId)?.escalationFlaggedAt"
                class="rounded bg-rose-100 px-1.5 py-0.5 text-[10px] font-semibold text-rose-700"
                :title="ts('st.inbox.tpHintTitle')"
              >{{ ts("st.inbox.escalatedBadge") }}</span>
            </div>
          </td>
          ...
          <td class="px-4 py-3 text-right">
            <button
              ...
              :disabled="['wait', 'limit', 'claimed'].includes(actionsById.get(item.applicationId)!)"
              :title="newActionTitle(actionsById.get(item.applicationId)!)"
