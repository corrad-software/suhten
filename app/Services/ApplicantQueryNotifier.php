...
/**
 * Sends email when staff raises a query (Pertanyaan) for the applicant.
 *
 * Delivery can be redirected via config('st.staff_mail_override') for local/demo
 * simulation — same override used by staff task emails.
 */
class ApplicantQueryNotifier
{
    ...
    public function notify(array $context): bool
    {
        ...
        if ($intendedEmail === '' || $note === '') {
            Log::warning('Applicant query email skipped: missing email or note', [
                'has_email' => $intendedEmail !== '',
                'has_note' => $note !== '',
        ...
        try {
            Mail::to($to)->send(new ApplicantQueryMail($payload));
            Log::info('Applicant query email sent', [
                'to' => $to,
            ...
            Log::error('Failed to send applicant query email', [
                'to' => $to,
                'intended_email' => $intendedEmail,
                'ref_no' => $refNo,
                'error' => $e->getMessage(),
    ...
    private function buildInternalPath(
        ...
        if (is_string($applicationCode) && $applicationCode !== '') {
            return '/st/applications/'.$applicationCode.'?stage=query_applicant';
        }
