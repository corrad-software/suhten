...
use App\Services\AnthropicUserChatService;
...
class UserChatController extends Controller
{
    ...
    public function __construct(
        protected AnthropicUserChatService $chatService,
    ...
    public function newUserChatSession(StoreUserChatSessionRequest $request): JsonResponse
    {
        ...
        $session = ChatSession::create([
            'openai_thread_id' => $this->chatService->createSessionKey(),
    ...
    public function sendUserChatMessage(SendUserChatMessageRequest $request, int $sessionId): JsonResponse
    {
        ...
        try {
            ...
            return $this->sendError(500, 'INTERNAL_ERROR', $this->normalizeAiError($e->getMessage()));
        }

        if (isset($response['error'])) {
            return $this->sendError(500, 'INTERNAL_ERROR', $this->normalizeAiError($response['error']));
        }
    ...
    {
        return [
            ['id' => 'lesen-elektrik', 'label' => 'Macam mana nak mohon lesen elektrik?', 'module' => 'Pelesenan Elektrik'],
            ['id' => 'ok-elektrik', 'label' => 'Cara daftar sebagai OK Elektrik?', 'module' => 'OK Elektrik'],
            ['id' => 'semak-status', 'label' => 'Bagaimana nak semak status permohonan?', 'module' => 'Carian & Semakan'],
            ['id' => 'lawatan-tapak', 'label' => 'Cara proses lawatan tapak / pemeriksaan?', 'module' => 'Lawatan Tapak'],
            ['id' => 'bayaran-fi', 'label' => 'Macam mana nak bayar fi dalam sistem?', 'module' => 'Pengurusan Hasil'],
        ];
    }

    private function normalizeAiError(string $raw): string
    {
        $lower = strtolower($raw);
        if (str_contains($lower, 'rate_limit') || str_contains($lower, 'rate limit') || str_contains($lower, 'too many requests')) {
            return 'Bukan masalah kredit. Terlalu banyak permintaan ke API AI (rate limit). Sila tunggu 2–3 minit dan cuba semula.';
        }
        if (str_contains($lower, 'credit') || str_contains($lower, 'quota') || str_contains($lower, 'billing')) {
            return 'Had kuota API Anthropic telah habis. Sila tambah kredit di dashboard Anthropic atau hubungi pentadbir sistem.';
        }
        ...
        if (str_contains($lower, 'overloaded') || str_contains($lower, 'server') || str_contains($lower, 'something went wrong')) {
